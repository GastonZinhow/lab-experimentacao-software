...
Dado o tamanho amostral reduzido:

- prefira **mediana e IQR (intervalo interquartil)** a média e desvio-padrão nas tabelas e gráficos descritivos;
- mantenha o **teste de Wilcoxon (não paramétrico)** na análise inferencial do Passo 4 — consistente com o desenho *within-subject*.
...
> Se as katas forem muito conhecidas — por exemplo, exercícios clássicos do LeetCode/HackerRank —, o assistente de IA pode reproduzir uma solução já vista em seu treinamento em vez de efetivamente "ajudar". Para reduzir esse risco, prefira katas autorais do grupo/professor ou exercícios pouco indexados.
...
Prepare o ambiente definindo:

- linguagem;
- IDE;
- assistente de IA a ser usado;
...
- versão gratuita de um chatbot como ChatGPT, Claude ou Gemini.

Fixe também a linguagem de programação das katas de acordo com a ferramenta de métricas estáticas escolhida:

- **Java:** CK / PMD;
- **Python:** Radon ou equivalente;
- outras linguagens: ferramenta equivalente.

### 3. Execução do Experimento

Cada integrante do grupo resolve:

- metade dos katas **com assistente de IA habilitado**;
- metade dos katas **sem assistente de IA**;
- em ordem **contrabalanceada** entre os integrantes;
- sob tempo limitado.

#### Time-box fixo

> **35 min/trial — só pode ser reduzido, nunca aumentado.**
...
- métricas obtidas pelo CK/PMD ou ferramenta equivalente sobre o código final de cada trial.
...
- **Teste de Wilcoxon para amostras pareadas**, dado o desenho *within-subject*.

### 5. Relatório Final

O documento final deve conter:

1. **Introdução**, incluindo as hipóteses;
2. **Metodologia**, detalhada o suficiente para permitir reprodução/replicação:
   - ambiente;
   - katas usados;
   - assistente de IA;
   - versão do assistente;
3. **Resultados por RQ**, incluindo as respostas estatísticas obtidas;
4. **Discussão final**;
...
Importe os dados do experimento e gere gráficos utilizando:
...
A contribuição é **individual por sprint**.

Em toda sprint (**S01, S02 e S03**), cada integrante do trio deve ser **Assignee de ao menos uma Issue com artefato de código commitado**, como:
...
Não basta participar apenas das Issues de execução de katas da S02.
...
A divisão abaixo **não é obrigatória**. O trio pode se organizar de outra forma, desde que a regra de contribuição individual seja respeitada.
...
A divisão já ocorre naturalmente pelo desenho experimental:
...
## GitHub Projects e rastreabilidade
...
- atribuída ao integrante responsável no campo **Assignee**;
